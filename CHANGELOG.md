...
- **`BBLink` forwards `onClick`** - it was accepted through `IPropsBBBase` and then never destructured, so it silently did nothing. It now goes on the anchor, which is both the click target and the only element whose type matches `MouseEventHandler<HTMLElement>`. Non-breaking for anyone who was not passing it, and it starts working for anyone who was.
- Cypress coverage for all three, plus `BBLink`'s `asSpan` (previously a TODO in that spec). The new label-markup assertions were checked against the old `BBButton` and fail at every size, and a regression test pins `className` to the inner text so a later change cannot quietly move it.
